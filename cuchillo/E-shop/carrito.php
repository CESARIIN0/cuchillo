...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['id']) ) {
        $productID = $_POST['id'];
        $num = $_POST['num'];
        $sql = "SELECT stock FROM plato WHERE id_Plato='$productID'";
        ...
        if ($result->num_rows > 0) {
            ...
            if ($resultados->num_rows > 0) {
                ...
                $sql = "UPDATE plato SET stock = $actualizar WHERE id_Plato = $productID";
            ...
            $sql = "SELECT * FROM carrito";
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        $sql2 = "SELECT stock FROM plato WHERE id_Plato='$RUTA'";
                        ...
                        if ($result2->num_rows > 0) {
                            ...
                            if ($resultados->num_rows > 0) {
                                ...
                                $nuevoSTOCK = $rows['stock'];
                                $actualizar = $original + $nuevoSTOCK;
                                $sql = "UPDATE plato SET stock = $actualizar WHERE id_Plato = $RUTA";
                                if ($conn->query($sql) === TRUE) {
                                    //echo "<script type='text/javascript'>alert('Actualizado');</script>";
                                } else {
                                    echo "Error: " . $sql . "<br>" . $conn->error;
...
<?php if ($aviso): ?>
  ...
  <div class="text">Ticket</div>
        ...
        <div class="modal-content">
            ...
            <p id="modal-message">Compra Realizada Exitosamente</p>
        </div>
    </div>
    </div>
